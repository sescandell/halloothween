...
export class PrinterClient {
    ...
    async _printViaCUPS(photoPath) {
        ...
        try {
            // FIX for "Filter Failed" (on original JPGs)
            // If the incoming file is already a PNG (e.g. from FrameComposer via /dev/shm), 
            ...
            // Cleanup temp file (conversion)
            if (tempFileCreated && fs.existsSync(fileToPrint)) {
                try {
                    fs.unlinkSync(fileToPrint);
                    console.log(`[PRINTER] Cleaned up temp conversion file: ${fileToPrint}`);
                } catch (e) {
                    console.error('[PRINTER] Failed to cleanup temp file:', e.message);
                }
            }

            // CLEANUP: If the input file was generated by FrameComposer (e.g. in /dev/shm or framed_),
            // we should delete it to avoid filling up RAM/Disk.
            // We identify these files by their directory (/dev/shm) or name pattern
            const isRamFile = photoPath.startsWith('/dev/shm');
            const isFramedFile = path.basename(photoPath).startsWith('print_job_') || path.basename(photoPath).startsWith('framed_');
            
            if ((isRamFile || isFramedFile) && fs.existsSync(photoPath)) {
                try {
                    fs.unlinkSync(photoPath);
                    console.log(`[PRINTER] Cleaned up print job file: ${photoPath}`);
                } catch (e) {
                    // Ignore errors if file is locked or already gone
                }
            }
        }
    }
